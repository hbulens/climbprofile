...
const ClimbProfileChart: React.FC<ClimbProfileProps> = ({ climbProfile, zoomLevel = 1 }) => {
    ...
    useEffect(() => {
        ...
        const margin = { top: 5, right: 60, bottom: 40, left: 40 };
        ...
        data.forEach((d, i) => {
            if (i < data.length - 1) {
                ...
                svg.append('text')
                    .attr('x', xScale(d.start) + margin.left - 0 + (xScale(next.start) - xScale(d.start)) / 2)
